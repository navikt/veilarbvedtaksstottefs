...
function LongParagraphs(props: { content: any }) {
	...
	const listLongParagraphs = longParagraphs.map((paragraph, index) => (
		<li key={index} className="pb-5">
			{paragraph}
			<b>({paragraph.replace(/([.?!])\s*(?=[A-Z])/g, '$1|').split('|').length}&nbsp;setninger)</b>
		</li>
	));
			...
			{longParagraphHere != 0 && (
				<Accordion.Item>
					...
					<Accordion.Content>
						...
						"Hvert avsnitt skal ha ett budskap og ikke inneholde mer enn 2-3 setninger." -{' '}
						<Link target="_blank" href="https://aksel.nav.no/artikkel/sprakarbeid?tema=innholdsarbeid">
							Aksel
							<ExternalLink title="Ekstern lenke" />
						</Link>
						<Heading className="mt-6" spacing level="3" size="xsmall">
							Avsnitt med over 3 setninger
						</Heading>
						<ul className="list-disc pt-5 list-inside">{listLongParagraphs}</ul>
					</Accordion.Content>
				</Accordion.Item>
			)}
		</>
