...
function PersonalData(props: { content: string }) {
	...
	// let getemail = <></>;
	// if (email) {
	// 	getemail = email.map((email, index) => (
	...
	// let getphone = <></>;
	// if (phone) {
	// 	getphone = phone.map((phone, index) => (
	// 		<li key={index} className="pb-2">
	// 			{phone}
	// 		</li>
	// 	));
					...
					<Accordion.Content className="removeAccordionPaddingBottom">
						{emailCount >= 1 && (
							<>
								...
								<ul className="list-disc pt-5 list-inside">{listEmail}</ul>
							</>
						)}
						{phoneCount >= 1 && (
							<>
								...
								<ul className="list-disc pt-5 list-inside">{listPhone}</ul>
							</>
						)}
					</Accordion.Content>
				</Accordion.Item>
