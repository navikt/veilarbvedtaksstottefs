...
const Innspill = ({
	faneNavn,
	fritekst,
	handleFaneNavnChanged,
	handleFritekstChanged,
	handleSendInnspillClicked
}: {
	faneNavn: FaneNavn | null;
	fritekst: string;
	handleFaneNavnChanged: (e: any) => void;
	handleFritekstChanged: (e: any) => void;
	handleSendInnspillClicked: () => void;
}) => (
	<Card className="innspill">
		<Innholdstittel className="innspill__tittel">Kom med dine innspill</Innholdstittel>
		<section className="innspill__navn">
			<label form="fanenavn-form">
				<Undertittel>Hva skal fanen hete?</Undertittel>
		...
		<section className="innspill__besvarelse">
			<Textarea
				value={fritekst}
				label={[
					<Undertittel key="undertittel">Hva lurer du på om den nye løsningen?</Undertittel>,
			...
			/>
			<Knapp onClick={handleSendInnspillClicked}>Send</Knapp>
		</section>
	</Card>
);
